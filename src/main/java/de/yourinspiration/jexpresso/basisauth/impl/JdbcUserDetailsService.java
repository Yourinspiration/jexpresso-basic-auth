...
import de.yourinspiration.jexpresso.basicauth.GrantedAuthority;
import de.yourinspiration.jexpresso.basicauth.UserDetails;
import de.yourinspiration.jexpresso.basicauth.UserDetailsService;
import de.yourinspiration.jexpresso.basicauth.UserNotFoundException;
...
public class JdbcUserDetailsService implements UserDetailsService {
    ...
    private Collection<GrantedAuthority> getGrantedAuthorities(final String username) {
        final String authoriyQuery = authoritiesByUsernameQuery.replace("?", "'" + username + "'");

        final Collection<GrantedAuthority> authorities = new ArrayList<>();

        try {
            final ResultSet authoriyResultSet = connection.createStatement().executeQuery(authoriyQuery);
